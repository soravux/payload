...
{
    // Interrupts should never be this long but since it's for slow speed
    // UART, it should be OK...
    *(GPSWPtr++) = getcUART1();
    // If buffer overflow or not 80 characters left to hold whole NMEA string
    if (GPSWPtr[-1] == NEWLINE && GPSState == GPS_BEGIN_FOUND) {
        GPSState = GPS_READY;
        GPSRPtr = GPSWPtr - 1;
    } else if (GPSWPtr[-1] == '$' && GPSState == GPS_INVALID) {
        GPSState = GPS_BEGIN_FOUND;
    }

    if (GPSWPtr >= GPSData + 240
    || (GPSWPtr[-1] == NEWLINE && GPSWPtr >= GPSData + 240 - 80)) {
        GPSWPtr = GPSData;
    }
